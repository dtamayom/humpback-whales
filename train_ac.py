...
def curvita(recall, precision, average_precision):
    m = True
    ...
    for i, color in zip(range(0, 43), colors):
...
def Validation(epoch):
    with torch.no_grad():
        ...
        precision["micro"], recall["micro"], _ = precision_recall_curve(np.array(tar).ravel(), np.array(pru).ravel())
        average_precision["micro"] = average_precision_score(tar, pru, average="micro")
        print('Average precision score, micro-averaged over all classes: {0:0.2f}'
      .format(average_precision["micro"]))

        m = curvita(recall, precision, average_precision)
